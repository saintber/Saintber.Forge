...
| v1.1    | 2026-02-13 | AI 法規限制調整                    | 因 AI 拒絕提供完整歌詞，改為即時節錄片段出題            |
...
- 使用者可自行輸入任意格式的歌單文字
- AI 根據歌手與歌名即時節錄歌詞片段（10 字以上完整句子，句數越少越好）作為題目
- 遊戲能隨機選取歌曲並以歌詞提示使用者猜歌名
- 使用者可反覆作答或直接選擇公佈答案
- 每次答題結束後能立即進入下一題
...
- 本功能不依賴使用者身份、權限或登入狀態
- 歌詞與歌曲資訊完全依賴 AI 推論結果
- **AI 因法規限制無法提供完整歌詞**，僅能即時節錄片段（10 字以上完整句子）
- 歌詞片段僅於出題時即時產生，不進行永久保存或快取
- 不以多頁跳轉為主要互動方式
